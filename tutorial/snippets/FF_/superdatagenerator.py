...
class Super2000Data:
    ...
    def selectUserLowers(self, env: int, user: str) -> list:
        '''
        param1: env, 0: dev02, 1: joy188
        param2: username
        return: user regesiter url list
    ...
    def getLottery(self, env: int, lottery_type_: int) -> list:
        '''
        ...
        lottery_type = lottery_type_dict[lottery_type_]
    ...
    def selectUserid(self, env: int, user: str) -> None:
        '''
        param1: env
    ...
    def select_issue(self, env: int, lotteryid: int) -> Union[tuple, None]: 
    ...
    def addReason(self, env: int , user: str, fund_type: int, cost: float) -> None:
        '''
        param1: env
        param2: account
    ...
    def autoDeposit(self, env: int, user: str, amount: float) -> None:
        ...
        return: None
        '''
        self.addReason(env, user, 7, amount) # fund_type is 7, for 人工加幣
        self.confirmReason(env, user, 1) # status is 1, for pass

        return None
    
    def getAwardGroupID(self, env: int, lotteryid: int):
    ...
    def ballContentAll2000(self, env: int, lotteryid: int, moneyunit: float, awardMode: int) -> dict:
        ...
        bet_params['orders'][0]['number'] = issue[0][0] if issue else None
        bet_params['orders'][0]['issueCode'] = issue[1][0] if issue else None
    ...
    def activity578Rank(self, start: str, end: str) -> dict:
        '''
        return: bet ranking list
        '''
        time_range = (start, end)
        conn = self.oracle(1) # activity systerm working at joy188 (env = 1)
        with conn.cursor() as cursor:
            sql = f'''SELECT
                    fcl.USER_ID,
                    uc.ACCOUNT,
                    -SUM(CASE WHEN fcl.REASON IN ('GM,DVCB,null,2','GM,DVCN,null,2') THEN (fcl.CT_DAMT - fcl.BEFORE_DAMT)/10000 ELSE 0 END) AS Bet,
                    SUM(CASE WHEN ctbr.THIRDLY_PLAT_NAME IN ('PT', 'FHX') THEN (ctbr.THIRDLY_EFFCT_COST/10000) ELSE ctbr.THIRDLY_EFFCT_COST END) AS TBet,
                    SUM(CASE WHEN ctbr.THIRDLY_PLAT_NAME IN ('PT', 'FHX') THEN (ctbr.THIRDLY_EFFCT_COST/10000) ELSE ctbr.THIRDLY_EFFCT_COST END)
                    -SUM(CASE WHEN fcl.REASON IN ('GM,DVCB,null,2','GM,DVCN,null,2') THEN (fcl.CT_DAMT - fcl.BEFORE_DAMT)/10000 ELSE 0 END) AS TotalBet
                FROM 
                    FUND_CHANGE_LOG fcl 
                        INNER JOIN USER_CUSTOMER uc ON fcl.USER_ID = uc.ID
                        INNER JOIN COLLECT_THIRDLY_BET_RECORD ctbr ON fcl.USER_ID = ctbr.USER_ID 
                WHERE 
                    fcl.GMT_CREATED BETWEEN TO_DATE('{time_range[0]}','yyyy-MM-dd HH24:mi:ss') AND TO_DATE('{time_range[1]}','yyyy-MM-dd HH24:mi:ss')
                GROUP BY 
                    uc.ACCOUNT, fcl.USER_ID
                ORDER BY 
                    TotalBet DESC
            '''
            cursor.execute(sql)
