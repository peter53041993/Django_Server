...
#-*- coding: utf-8 -*-
import unittest,requests,hashlib,time,random,cx_Oracle,json
...
class Joy188Test(unittest.TestCase):
    ...
    def test_transferin():#第三方轉入
        ...
        for third in statu_list:
            ...
            if staut_mapping == '2':
                ...
                tocheck.append(third)
                msg = msg +" 狀態待確認"
            sleep(3)
            print(msg)
            
        
        print('需確認', tocheck if len(tocheck) > 0 else None)
        Joy188Test.test_thirdBalance()
        
        assert(len(tocheck) == 0)
    @staticmethod
    ...
    @staticmethod
    def test_super2000(env, account):
        
        fail = []
        
        amount = Super2000Data().getUserAmount(env, account)
        if amount <= 10000:
            Super2000Data().autoDeposit(env, account, 20000)
            print(f'{account} 餘額不足10000, 已成功加值20000')
    
        
        super2000_list = super_betcontent.lottery_2000
        
        Pc_header['Cookie']= 'ANVOID='+ cookies_[account]
        #Pc_header['Cookie']= 'ANVOID='+ FF_().cookies[account]
        print(f'投注帳號:{account}')
        
        for i in super_betcontent.lottery_2000:
            lottery = super2000_list[i][1]
            data = Super2000Data().ballContentAll2000(env, i, 1, 1)
            
            r = session.post(em_url+'/gameBet/'+lottery+'/submit', data = json.dumps(data), headers=Pc_header)
            
            
            if r.json()['isSuccess'] != 1:
                print(f'{super2000_list[i][2]}投注失敗，開始進行玩法檢驗')
                
                single_list = Super2000Data().ballContentSingle2000(env, i, 1, 1)
                
                for content in single_list:
                    data = content
                    r = session.post(em_url+'/gameBet/'+lottery+'/submit', data = json.dumps(data), headers=Pc_header)
                    
                    if r.json()['isSuccess'] != 1:
                        if r.json()['msg'] == '您好，当前彩种已停售！':
                            print(f'{data["balls"][0]["type"]}, {r.json()["msg"]}')
                        else:    
                            print(f'{data["balls"][0]["type"]}投注失敗')
                            fail.append(1)
                    else:
                        pass
                    
            
            elif r.json()['isSuccess'] == 1:
                print(f'{super2000_list[i][2]}投注成功')
        
        assert(len(fail) == 0)
                
        
    
    def test_Spuer2000Submit(self):
        u"投注測試"
        Joy188Test.test_super2000(account = env_dict['合營1940'][envs],env = envs)#env_dict['一般帳號'][envs],moneyunit=1,))#
    
    
    
    def testAllLottery(env, account):
        amount = Super2000Data().getUserAmount(env, account)
        if amount <= 10000:
            Super2000Data().autoDeposit(env, account, 20000)
            print(f'{account} 餘額不足10000, 已成功加值20000')
        ...
        for i in super_betcontent.lottery_all:
            if len(super_betcontent.lottery_all[i]) == 4:
                lottery_list_all.append(i)
        
        
        lottery_ccs_list = Super2000Data().getLottery(env, 0)
        
        #lottery_list = lottery_ccs_list
        print(f'投注彩種： {lottery_list}')
        
        for i in lottery_list:
            lottery = super_betcontent.lottery_all[i][1]
            data_list = Super2000Data().ballContentAll(env, i, 1, 1)
            
            for j in range(len(data_list)):
                gameplay_type = super_betcontent.lottery_all[i][3][j]
                data = data_list[j]
                           
                r = session.post(em_url+'/gameBet/'+lottery+'/submit', data = json.dumps(data), headers=Pc_header)
                           
                if r.json()['isSuccess'] != 1:
                    print(f'{super_betcontent.lottery_all[i][2]}, 玩法分類{gameplay_type} 投注失敗，開始進行玩法檢驗')
                      
                    single_list = Super2000Data().ballContentSingle(env, i, 1, 1)
                
                    for k in single_list:
                           for content in k:
                                r = session.post(em_url+'/gameBet/'+lottery+'/submit', data = json.dumps(content), headers=Pc_header)
                                if r.json()['isSuccess'] != 1:
                                    print(f'{content["balls"][0]["type"]} 投注失敗')
                               
                               
                                
                elif r.json()['isSuccess'] == 1:
                    print(f'{super_betcontent.lottery_all[i][2]}, 玩法分類{gameplay_type} 投注成功')
    
    def testAllLotterySubmit(self):
        u"投注測試"
        Joy188Test.testAllLottery(account = env_dict['合營1940'][envs], env = envs)
...
class Joy188Test2(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            cls.dr = webdriver.Chrome(executable_path=r'C:\\Users\\Peter\\drf_test\\tutorial\\snippets\\FF_\\chromedriver')
            dr = cls.dr
            post_url = url_dict[envs][0]
            if envs == 1: 
                cls.dr.get(post_url)
