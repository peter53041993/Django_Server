...
class OracleConnection:
    ...
    def select_fee(self, type_, user):  # 查詢 用戶 總代線 手續費
        ...
        cursor.close()
        return fund_fee
    def select_withdrawApply(self,sn='',type_='',userid='',new_time=''):#單號 查詢申請時間,預設 帶''.用last查詢上個訂單的時間
        cursor = self._get_oracle_conn().cursor()
        if type_ == '':
            sql = f"select ID,user_id,to_char(apply_time,'YYYY-MM-DD HH24:MI:SS'),root_sn from FUND_WITHDRAW where sn = '{sn}'"# ID 用意 抓出來, 在用這個ID -1 抓上一個提現單的時間
        elif type_ == 'last':# 這裡sn 不是頁面輸入的sn , 是 root_sn
            sql = f"select to_char(apply_time,'YYYY-MM-DD HH24:MI:SS') from FUND_WITHDRAW where id = " \
            f"(select max(id) from FUND_WITHDRAW  where user_id = {userid} and root_sn != '{sn}' )"# 拆單話 sn都一樣. 所以需用root_sn
        else:
            sql =f"select to_char( to_date('{new_time}','YYYY-MM-DD HH24:MI:SS') -  90 ,'YYYY-MM-DD HH24:MI:SS' ) from dual "
            cursor.execute(sql)
            rows = cursor.fetchall()
            last_date = [ i for i in rows[0]][0]
            return last_date 
        ...
        rows = cursor.fetchall()
        withdraw = []
        if len(rows) == 0:# 這裡是 抓上一筆提現單, 有可能沒有 新用戶/或者第一次提現
            if type_ == '':# 這邊是   輸入錯單號
                return 'none'
            else:
                return [0]
        ...
        cursor.close()
        return withdraw 
    def select_withProfit(self,userid,new_time,last_time,type_=''):# 用提縣的 時間區間,來抓取 用戶遊戲紀錄 盈虧.type_ 預設 投注相關數據
        cursor = self._get_oracle_conn().cursor()
        '''
        last_time 為 上一筆提現單時間, new_time 為當下發起單號的時間
        '''
        sql = f"select sum(slip.totamount)/10000 投注, sum(slip.EVALUATE_WIN)/10000 中獎, sum(slip.RET_POINT)/10000 反點, " \
        f"sum(slip.EVALUATE_WIN)/10000 +sum(slip.RET_POINT)/10000 - sum(slip.totamount)/10000 盈利 " \
        f"from game_slip slip  join game_order order_ on slip.orderid = order_.id  and slip.status = order_.status " \
        f"where order_.status not in (1,4)  and order_.userid  = {userid} and  order_.order_time > " \
        f"to_date('{last_time}','YYYY-MM-DD HH24:MI:SS') and order_.order_time < to_date('{new_time}','YYYY-MM-DD HH24:MI:SS') " \
        f"group by  order_.userid"
        print(sql)
        cursor.execute(sql)
        rows = cursor.fetchall()
        order_profit = {}
        if len(rows) == 0:# 沒值得處理
            if type_ == 'WithDraw3':#風控3
                return '0'
            elif type_ == 'WithDraw2':#風控2 
                order_profit['營利'] = 0
            else:# 風控1
                order_profit['投注'] = 0
                order_profit['中獎'] = 0
        ...
        cursor.close()
        return order_profit
    def select_withDeposit(self,userid,last_time,new_time):# 用 提現單 抓取 成功充值金額
        cursor = self._get_oracle_conn().cursor()
        sql = f"select NVL(sum(REAL_CHARGE_AMT-CHARGE_FEE)/10000,0) from fund_charge " \
        f"where user_id = '{userid}' and charge_time  between " \
        f"to_date('{last_time}','YYYY-MM-DD HH24:MI:SS') and to_date('{new_time}','YYYY-MM-DD HH24:MI:SS') and STATUS = 2" 
        #print(sql)
        cursor.execute(sql)
        rows = cursor.fetchall()
        depoist = []
    ...
    def select_withRisk(self,userid,apply_time,sn):#查詢 風控 上現後,該用戶 是否 > 1 筆
        cursor = self._get_oracle_conn().cursor()
        sql = f"select count(id) from FUND_WITHDRAW_RISK where user_id = {userid} and apply_time < " \
        f"to_date('{apply_time}','YYYY-MM-DD HH24:MI:SS') and sn != '{sn}' "
        print(sql)
        cursor.execute(sql)
        rows = cursor.fetchall()
        len_risk = []
        ...
        return len_risk[0]
    def select_withLoginBalance(self,sn,userid):# 提現單區間 抓取 4.0/三方 餘額/ 登入時間 ,sn 需使用 root_sn ,如果遇到大額拆單的sn會有問題 
        cursor = self._get_oracle_conn().cursor()
        sql = f"select to_char(max(login.login_date),'YYYY-MM-DD HH24:MI:SS'),fund.FF_LESS_AMOUNT,fund.THIRDLY_LESS_AMOUNT,max(login.ID) "\
        f"from  USER_LOGIN_LOG login  join FUND_WITHDRAW_RISK fund "\
        f"on fund.user_id =  login.user_id "\
        f"where fund.sn = '{sn}' and login.user_id = '{userid}' "\
        f"group by fund.FF_LESS_AMOUNT,fund.THIRDLY_LESS_AMOUNT "
        print(sql)
        cursor.execute(sql)
        rows = cursor.fetchall()
        LoginBalance = []
        for i in rows[0]:
            LoginBalance.append(i)
        cursor.close()
        return LoginBalance
    
    def select_BcProfit(self,userid,last_time,new_time):# 風控 ,抓取BC  投注/中獎 'GM,BCDB,null,2','GM,BCAW,null,1'(投注, 中獎)
        cursor = self._get_oracle_conn().cursor()
        sql = f"SELECT (before_damt - ct_damt)/10000 投注, (ct_bal - befor_bal)/10000 獎金 "\
        f"FROM fund_change_log WHERE reason in ('GM,BCDB,null,2','GM,BCAW,null,1') "\
        f"and user_id = {userid} and gmt_created between to_date('{last_time}','YYYY-MM-DD HH24:MI:SS') "\
        f"and to_date('{new_time}','YYYY-MM-DD HH24:MI:SS')"
        #print(sql)
        cursor.execute(sql)
        rows = cursor.fetchall()
        bc_profit = defaultdict(list)
        if len(rows) == 0:
            bc_profit['BC投注'] = [0]
            bc_profit['BC中獎'] = [0]
            return bc_profit# 投注. 中獎
        for i in rows:
            bc_profit['BC投注'].append(i[0])
            bc_profit['BC中獎'].append(i[1])
        cursor.close()
        return bc_profit

    def select_lastlogin(self,id_,userid): # 查詢上次登入的時間 ,風控 提款信息变更 用的
        cursor = self._get_oracle_conn().cursor()
        sql = f"select  NVL(to_char(max(login_date),'YYYY-MM-DD HH24:MI:SS'),0) from USER_LOGIN_LOG where id != {id_} "\
    ...
    def select_thirdTrans(self,userid,last_time,new_time):# 查詢三方 轉入/轉出
        cursor = self._get_oracle_conn().cursor()
        third_trans = defaultdict(list)
        transfer_dict = {0:[2,'(before_damt - ct_damt)/10000 轉入','轉入'],1:[6,'(ct_bal - befor_bal)/10000 轉出','轉出']}
        for i in range(2):
            sql = f"SELECT  {transfer_dict[i][1]} ,reason  " \
            f"FROM fund_change_log WHERE user_id = {userid} "\
            f"AND gmt_created between TO_DATE('{last_time}','YYYY-MM-DD HH24:MI:SS') " \
            f"and TO_DATE('{new_time}','YYYY-MM-DD HH24:MI:SS') " \
            f"and reason like '%TF%' and reason like '%{transfer_dict[i][0]}%' order by gmt_created desc"
            print(sql)
            cursor.execute(sql)
            rows = cursor.fetchall()
            if len(rows) == 0:
                third_trans[transfer_dict[i][2]].append(0)
                third_trans['%sreason'%transfer_dict[i][2]].append('no')
            else:
                for a in rows:
                    third_trans[transfer_dict[i][2]].append(a[0])
                    third_trans['%sreason'%transfer_dict[i][2]].append(a[1])
    ...
    def select_fundbalance(self,userid,last_time,new_time):#確認 兩筆題現過程中,有無餘額小於1 的清零點, HB,DHBS 紅包 的餘額 非FF餘額
        cursor = self._get_oracle_conn().cursor()
        sql = f"select NVL(to_char(max(gmt_created),'YYYY-MM-DD HH24:MI:SS'),0) from fund_change_log "\
        f"where USER_ID = {userid} "\
        f"and ct_bal <10000 and reason not like '%HB,DHBS%' and  gmt_created between "\
        f"to_date('{last_time}','YYYY-MM-DD HH24:MI:SS') and to_date('{new_time}','YYYY-MM-DD HH24:MI:SS')"#ct_bal 餘額 , 10000 = 1
        print(sql)
        cursor.execute(sql)
        rows = cursor.fetchall()
        zero_balance = [ ]
        for i in rows[0]:
            zero_balance.append(i)
        cursor.close()
        return zero_balance[0]


    def select_loginbank(self,last_logindate,userid,new_time):#用最後一次登入時間 查詢用戶是否有綁卡操作行為
        cursor = self._get_oracle_conn().cursor()
        sql = f"select action from view_USER_BANK_BIND_HISTORY where user_id = {userid} and action_time between "\
        f"to_date('{last_logindate}','YYYY-MM-DD HH24:MI:SS') and to_date('{new_time}','YYYY-MM-DD HH24:MI:SS')"
        print(sql)
        cursor.execute(sql)
        rows = cursor.fetchall()
        bank_action = []
        if len(rows) == 0:
            return [0]
        for i in rows:
            bank_action.append(i[0])
        cursor.close()
        return bank_action

    def select_WithDrawControl(self):#提現風控設定
        cursor = self._get_oracle_conn().cursor()
        sql = "SELECT value FROM CONFIG WHERE KEY='fhWithdrawRiskConfig' "
        cursor.execute(sql)
        rows = cursor.fetchall()
        risk_setting = {}
        for i in rows[0]:
            #print(i)
            for key,value in eval(i).items():# i str 轉成 dict
                risk_setting[key] = value
        cursor.close()
        return risk_setting
